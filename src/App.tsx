...
import { buildShotTimeRanges, projectDuration } from "./utils/time";
import { checkDesktopUpdate, installDesktopUpdate } from "./api/desktop";

export default function App() {
  const { state, selectedShot, dispatch, exportJson } = useStoryboardProject();
  ...
  const selectShot = useCallback((shotId: string) => {
    ...
    if (range) setGlobalTimeMs(range.startMs);
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      const next = startOffsetRef.current + (now - start);
      if (next >= totalDuration) {
        setGlobalTimeMs(totalDuration);
        setPlaying(false);
        startedAtRef.current = null;
        return;
      }
  ...
  useEffect(() => {
    const handleKey = (event: KeyboardEvent) => {
      ...
      if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "d" && selectedShot) {
        event.preventDefault();
        dispatch({ type: "duplicate", shotId: selectedShot.id });
      }
    ...
    <div className="app-shell">
      ...
      <div className="workspace">
        <ShotList
          ...
          selectedId={state.selectedShotId}
          onSelect={selectShot}
          onAdd={() => dispatch({ type: "add" })}
          onDuplicate={(shotId) => dispatch({ type: "duplicate", shotId })}
          onArchive={(shotId) => dispatch({ type: "archive", shotId })}
          onMove={(shotId, direction) => dispatch({ type: "move", shotId, direction })}
        />
        <PreviewPlayer
          shot={selectedShot}
          aspectRatio={state.project.aspectRatio}
          playing={playing}
          progress={Math.min(shotProgress, selectedShot?.durationMs ?? 0)}
          onTogglePlay={togglePlay}
        />
        <Inspector
          shot={selectedShot}
          onUpdate={(patch) => selectedShot && dispatch({ type: "update", shotId: selectedShot.id, patch })}
          onAddSfx={() => selectedShot && dispatch({ type: "addSfx", shotId: selectedShot.id })}
          onUpdateSfx={(sfxId, patch) => selectedShot && dispatch({ type: "updateSfx", shotId: selectedShot.id, sfxId, patch })}
          onRemoveSfx={(sfxId) => selectedShot && dispatch({ type: "removeSfx", shotId: selectedShot.id, sfxId })}
        />
      </div>
      <Timeline
        shots={state.project.shots}
