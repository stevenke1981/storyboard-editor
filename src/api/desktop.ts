...
export async function resolveMediaSource(path: string): Promise<string> {
  ...
  const absolutePath = await invokeTauri<string>("resolve_workspace_media", { relativePath: path });
  const { convertFileSrc } = await import("@tauri-apps/api/core");
  return convertFileSrc(absolutePath);
}

export function createDesktopProject(root: string, title: string): Promise<string> { return invokeTauri<string>("new_project", { root, title }); }
export function openDesktopProject(projectRoot: string): Promise<StoryboardProject> { return invokeTauri<StoryboardProject>("open_project", { projectRoot }); }
...
export function installDesktopUpdate(confirmed: boolean): Promise<string> { return invokeTauri<string>("install_update", { confirmed }); }
