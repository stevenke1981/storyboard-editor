...
function normalizeOrder(project: StoryboardProject): StoryboardProject {
  return {
    ...
    shots: project.shots.map((shot, index) => ({ ...shot, order: index + 1 })),
  };
}
...
function reduceProject(state: ProjectState, action: Exclude<ProjectAction, { type: "undo" } | { type: "redo" }>): ProjectState {
  ...
  switch (action.type) {
    case "select":
      return { ...state, selectedShotId: action.shotId };
    case "add": { 
    ...
    case "duplicate": {
      ...
      const clone: Shot = {
        ...
        createdAt: timestamp,
        updatedAt: timestamp,
        soundEffects: source.soundEffects.map((sfx) => ({ ...sfx, id: createId("sfx") })),
      };
      const shots = [...state.project.shots];
    ...
    case "archive": {
      const shots = state.project.shots.map((shot) => shot.id === action.shotId
        ? { ...shot, status: "archived" as const, enabled: false, updatedAt: timestamp }
        : shot);
      const next = shots.find((shot) => shot.id !== action.shotId && shot.status !== "archived");
      return {
        ...state,
        project: normalizeOrder({ ...state.project, shots }),
        selectedShotId: next?.id ?? action.shotId,
        dirty: true,
      };
    }
    case "update": {
      const shots = state.project.shots.map((shot) => shot.id === action.shotId
        ? { ...shot, ...action.patch, updatedAt: timestamp }
        : shot);
      return { ...state, project: { ...state.project, shots, updatedAt: timestamp }, dirty: true };
    }
    case "addSfx": {
      const sfx: SoundEffect = {
        id: createId("sfx"), name: "新音效", path: "", startMs: 0, trimInMs: 0,
        trimOutMs: null, gainDb: 0, loop: false, fadeInMs: 0, fadeOutMs: 0,
      };
      const shots = state.project.shots.map((shot) => shot.id === action.shotId
        ? { ...shot, soundEffects: [...shot.soundEffects, sfx], updatedAt: timestamp }
        : shot);
      return { ...state, project: { ...state.project, shots, updatedAt: timestamp }, dirty: true };
    }
    case "updateSfx": {
      const shots = state.project.shots.map((shot) => shot.id === action.shotId
        ? {
            ...shot,
            soundEffects: shot.soundEffects.map((sfx) => sfx.id === action.sfxId ? { ...sfx, ...action.patch } : sfx),
            updatedAt: timestamp,
          }
        : shot);
      return { ...state, project: { ...state.project, shots, updatedAt: timestamp }, dirty: true };
    ...
    case "removeSfx": {
      const shots = state.project.shots.map((shot) => shot.id === action.shotId
        ? { ...shot, soundEffects: shot.soundEffects.filter((sfx) => sfx.id !== action.sfxId), updatedAt: timestamp }
        : shot);
      return { ...state, project: { ...state.project, shots, updatedAt: timestamp }, dirty: true };
    }
    case "move": {
    ...
    case "replace":
      return { ...state, project: action.project, selectedShotId: action.project.shots[0]?.id ?? "", dirty: false, past: [], future: [] };
    case "clean":
...
function reducer(state: ProjectState, action: ProjectAction): ProjectState {
  if (action.type === "undo" || action.type === "redo") {
    ...
    const project = source[source.length - 1];
    const current = state.project;
    return { ...state, project, selectedShotId: project.shots.find((s) => s.status !== "archived")?.id ?? project.shots[0]?.id ?? "", dirty: true,
...
function loadInitialState(): ProjectState {
  try {
    ...
    if (raw) {
      const project = JSON.parse(raw) as StoryboardProject;
      return { project, selectedShotId: project.shots.find((s) => s.status !== "archived")?.id ?? "", dirty: false, past: [], future: [] };
    }
  } catch {
    // Use a safe demo when browser storage is invalid.
  }
...
export function useStoryboardProject() {
  ...
  const selectedShot = useMemo(
    () => state.project.shots.find((shot) => shot.id === state.selectedShotId) ?? null,
    [state.project.shots, state.selectedShotId],
  );
  ...
  const exportJson = useCallback(() => {
    ...
    anchor.href = url;
    anchor.download = "storyboard.json";
    anchor.click();
    URL.revokeObjectURL(url);
  }, [state.project]);

  return { state, selectedShot, dispatch, exportJson };
}
